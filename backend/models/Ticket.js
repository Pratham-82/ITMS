...
const TicketSchema = new mongoose.Schema({
  ...
  feedback: {
    ...
    resolvedCompletely: { type: String },
...
// Pre-save hook to generate sequential trackingId and validate mappings
TicketSchema.pre('save', async function (next) {
  // Update status based on assignment: Assigned if has group or user, Pending if none
  if (this.status === 'Pending' || this.status === 'Assigned') {
    if (this.assignedGroup || this.assignedTo) {
  ...
  if (this.isNew) {
    try {
      ...
      const ticketTypeDoc = await TicketType.findById(this.ticketType);
      
      const prefixCode = ticketTypeDoc ? ticketTypeDoc.code : 'CMS';
      const activeTenant = tenantLocalStorage.getStore() || this.tenantId || 'default-tenant';
      this.tenantId = activeTenant;
      let counterKey = `ticketTrackingId_${activeTenant}_${prefixCode.toLowerCase()}`;
      if (prefixCode === 'CMS') {
        counterKey = `complaintTrackingId_${activeTenant}`; // preserve sequence for complaints per tenant
      }
      ...
      this.trackingId = `${prefixCode}-${counter.seq}`;
      
      if (this.history.length === 0) {
        this.history.push({
          action: 'Ticket registered successfully',
          actor: 'System'
        });
      }
...
// Pre-save hook to handle SLA Pausing and Resuming
TicketSchema.pre('save', async function (next) {
...
TicketSchema.index({ currentEscalationLevel: 1 });
TicketSchema.index({ responseDueAt: 1 });
TicketSchema.index({ resolutionDueAt: 1 });
TicketSchema.index({ nextEscalationDueAt: 1 });
TicketSchema.index({ createdAt: -1 });
TicketSchema.index({ duplicateGroupId: 1 });
